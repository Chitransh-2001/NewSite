import { initializeApp, cert, getApps } from 'firebase-admin/app';
import { getAuth } from 'firebase-admin/auth';

const project = "adminboard-db0c6";
const client = "firebase-adminsdk-fbsvc@adminboard-db0c6.iam.gserviceaccount.com"; 
const private_key = "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n";
const privateId = private_key.replace(/\\n/g, '\n');


if (!getApps().length) {  
  initializeApp({
    credential: cert({
      projectId: project,
      clientEmail: client,
      privateKey: privateId,
    }),
  });
}

export async function POST(req) {
  try {
    const { token } = await req.json();
    const decodedToken = await getAuth().verifyIdToken(token);

    const nowInSeconds = Math.floor(Date.now() / 1000);
    const issuedAt = decodedToken.iat; // Issued At
    const maxAge = 3 * 60; // 30 minutes in seconds

    if (nowInSeconds - issuedAt > maxAge) {
      return new Response(JSON.stringify({ success: false, message: "Token expired (30m limit)" }), { status: 401 });
    }

    return new Response(JSON.stringify({ success: true, uid: decodedToken.uid }), { status: 200 });

  } catch (error) {
    console.error("Token verification failed:", error);
    return new Response(JSON.stringify({ success: false, message: "Invalid token" }), { status: 401 });
  }
}

